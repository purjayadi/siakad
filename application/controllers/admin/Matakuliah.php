...
class Matakuliah extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'admin/matakuliah/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'admin/matakuliah/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'admin/matakuliah/index.html';
            $config['first_url'] = base_url() . 'admin/matakuliah/index.html';
        }
        ...
        $this->load->view('qmatakuliah/qmatakuliah_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Modelmatakuliah->get_by_id($id);
        if ($row) {
            $data = array(
		'kode_matakuliah' => $row->kode_matakuliah,
		'nama_matakuliah' => $row->nama_matakuliah,
        'sks' => $row->sks,
		'table_dosen_id_dosen' => $row->table_dosen_id_dosen,
		'table_blok_id_blok' => $row->table_blok_id_blok,
		'table_semester_id_semester' => $row->table_semester_id_semester,
	    );
            $this->load->view('matakuliah/table_matakuliah_read', $data);
        } else {
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('admin/matakuliah/create_action'),
	    'kode_matakuliah' => set_value('kode_matakuliah'),
	    'nama_matakuliah' => set_value('nama_matakuliah'),
        'sks' => set_value('sks'),
	    'table_dosen_id_dosen' => set_value('table_dosen_id_dosen'),
	    'table_blok_id_blok' => set_value('table_blok_id_blok'),
	    'table_semester_id_semester' => set_value('table_semester_id_semester'),
	);
        $this->load->view('matakuliah/table_matakuliah_form', $data);
    }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
		'kode_matakuliah' => $this->db->escape_str($this->input->post('kode_matakuliah',TRUE)),
		'nama_matakuliah' => $this->db->escape_str($this->input->post('nama_matakuliah',TRUE)),
        'sks' => $this->db->escape_str($this->input->post('sks',TRUE)),
		'table_dosen_id_dosen' => $this->db->escape_str($this->input->post('table_dosen_id_dosen',TRUE)),
		'table_blok_id_blok' => $this->db->escape_str($this->input->post('table_blok_id_blok',TRUE)),
		'table_semester_id_semester' => $this->db->escape_str($this->input->post('table_semester_id_semester',TRUE)),
	    );
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('admin/matakuliah/update_action'),
        ...
        'sks' => set_value('nama_matakuliah', $row->sks),
		'table_dosen_id_dosen' => set_value('table_dosen_id_dosen', $row->table_dosen_id_dosen),
		'table_blok_id_blok' => set_value('table_blok_id_blok', $row->table_blok_id_blok),
    ...
    public function update_action() 
    {
		...
		'table_semester_id_semester' => $this->db->escape_str($this->input->post('table_semester_id_semester',TRUE)),
            ...
            $this->Modelmatakuliah->update($this->input->post('kode_matakuliah', TRUE), $data);
	...
	$this->form_validation->set_rules('table_dosen_id_dosen', 'table dosen id dosen', 'trim|required');
	$this->form_validation->set_rules('table_blok_id_blok', 'table blok id blok', 'trim|required');
    ...
    public function excel()
    {
        ...
        $namaFile = "table_matakuliah.xls";
        $judul = "table_matakuliah";
        $tablehead = 0;
        $tablebody = 1;
        $nourut = 1;
        //penulisan header
	...
	xlsWriteLabel($tablehead, $kolomhead++, "Table Dosen Id Dosen");
	xlsWriteLabel($tablehead, $kolomhead++, "Table Blok Id Blok");
	xlsWriteLabel($tablehead, $kolomhead++, "Table Semester Id Semester");

	foreach ($this->Modelmatakuliah->get_all() as $data) {
            $kolombody = 0;
	    ...
	    xlsWriteNumber($tablebody, $kolombody++, $data->table_dosen_id_dosen);
	    xlsWriteNumber($tablebody, $kolombody++, $data->table_blok_id_blok);
